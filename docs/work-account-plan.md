...
Planned 2026-09-05. Not built yet. This is a plan, not design: the design is
[docs/DESIGN.md](DESIGN.md) and the vocabulary is [CONTEXT.md](../CONTEXT.md).
...
- A new decision in [docs/DESIGN.md](DESIGN.md), beside ADR-0005: a listing
  spans accounts and an id names one.
- Amend ADR-0023: the bubble keyword is per account, not the Primary's.
- Make sure the new decision names the commands it adds, so the registry and the
  skill stay the one place a command exists (ADR-0020).
